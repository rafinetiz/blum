  ...
  function S() {
    return (
      (p === null || p.byteLength === 0) &&
        (p = new Uint8Array(_.memory.buffer)),
      p
    );
  }
  const O =
...
${e.stack}`
  ...
  function L(e, n) {
    return (
      (_ = e.exports), (R.__wbindgen_wasm_module = n), (l = null), (p = null), _
    );
  }
  async function R(e) {
    ...
    const { instance: t, module: r } = await D(await fs.readFile('game_wasm_bg-BnV071fP.wasm'), n);
    return L(t, r);
  }
  let x;
  const V = async () => {
    x === void 0 && (x = R()), await x;
  };
